...
void testRosettaDifferentialSemantics() {
    ...
    {
        auto testCase = make("and32_memory", CaseId::and32_memory,
                             differentialBytes_and32_memory);
        bindMemory(testCase, rosa::x86::Register::Rdi, 0);
        testCase.request.state.rax = 0xFFFFFFFFF0F0F0F0ULL;
        const std::uint32_t value = 0x0FF00FF0U;
        std::memcpy(testCase.request.memory.data() + 4, &value,
                    sizeof(value));
    ...
    {
        auto testCase = make("movsx8_memory_positive",
                             CaseId::movsx8_memory_positive,
    ...
    {
        auto testCase = make("movsx8_memory_negative",
                             CaseId::movsx8_memory_negative,
    ...
    {
        auto testCase = make("cmovb32_taken", CaseId::cmovb32_taken,
                             differentialBytes_cmovb32_taken);
    ...
    {
        auto testCase = make("cmovb32_not_taken",
                             CaseId::cmovb32_not_taken,
...
void testMovsxGuestByteTo32BitRegister() {
    ...
    expect(memory.base == rosa::x86::Register::Rdi && memory.width == 8 &&
               memory.displacement == 0,
           "MOVSX byte memory source differs");
    expect(rosa::debug::dumpX86(decoded).find(
               "movsx edx, byte [rdi]") != std::string::npos,
    ...
    expectEqual(positiveState.rdi, source.value,
                "positive MOVSX byte memory changed its base");
    expectEqual(positiveState.rflags, std::uint64_t{0x8D7},
...
void testAnd32BitRegisterWithGuestMemory() {
    ...
    expectEqual(decoded[0].length, std::uint8_t{3},
                "AND r32, dword [base+disp8] length differs");
    const auto destination =
        std::get<rosa::x86::RegisterOperand>(decoded[0].operands[0]);
    const auto memory =
        std::get<rosa::x86::MemoryOperand>(decoded[0].operands[1]);
    ...
    expect(memory.base == rosa::x86::Register::Rdi && memory.hasBase &&
               !memory.ripRelative && memory.displacement == 4 &&
               memory.width == 32,
           "AND r32, dword [base+disp8] memory operand differs");
    expect(rosa::debug::dumpX86(decoded).find(
               "and eax, dword [rdi+0x4]") != std::string::npos,
    ...
    addressSpace.mapAnonymous(page, rosa::guest::guestPageSize,
                              rosa::guest::Permission::Read |
                                  rosa::guest::Permission::Write);
    constexpr std::array<std::uint8_t, 8> sourceAndSentinel{
        0xF0, 0x0F, 0xF0, 0x0F, 0xEF, 0xBE, 0xAD, 0xDE};
    ...
    expectEqual(state.rdi, page.value,
                "AND r32, dword [base+disp8] changed its base");
    expect(addressSpace.readBytes(source, sourceAndSentinel.size()) ==
               std::vector<std::uint8_t>(sourceAndSentinel.begin(),
                                         sourceAndSentinel.end()),
...
void testConditionalMoveBelow32() {
    ...
    expect(rosa::debug::dumpX86(decoded).find("cmovb ecx, eax") !=
               std::string::npos,
    ...
    expectEqual(taken.rax, std::uint64_t{0xAABBCCDD11223344ULL},
                "CMOVB r32 changed its source");
    expectEqual(taken.rflags, std::uint64_t{0x8D7 | 1U},
                "taken CMOVB r32 changed flags");
    ...
    expectEqual(notTaken.rax, UINT64_MAX,
                "untaken CMOVB r32 changed its source");
    expectEqual(notTaken.rflags, std::uint64_t{0x8D6 & ~std::uint64_t{1}},
