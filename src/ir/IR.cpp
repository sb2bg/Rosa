...
void Builder::unpackHighPackedSingleXmm(x86::XmmRegister destination,
                                        x86::XmmRegister source,
                                        guest::GuestAddress rip) {
    block_.operations.push_back(Operation{
        .opcode = Opcode::UnpackHighPackedSingleXmm,
        .width = Width::I64,
        .guestRip = rip,
        .guestXmmRegister = destination,
        .sourceGuestXmmRegister = source,
...
void Builder::unpackHighGuestPackedSingleXmm(ValueId address,
                                             x86::XmmRegister destination,
                                             guest::GuestAddress rip) {
    block_.operations.push_back(Operation{
        .opcode = Opcode::UnpackHighGuestPackedSingleXmm,
        .width = Width::I64,
        .guestRip = rip,
        .lhs = address,
        .guestXmmRegister = destination,
...
void Builder::convertIntToDoubleXmm(ValueId value, x86::XmmRegister destination,
                                    Width width, guest::GuestAddress rip) {
    block_.operations.push_back(Operation{
        .opcode = Opcode::ConvertIntToDoubleXmm,
        .width = width,
        .guestRip = rip,
        .lhs = value,
        .guestXmmRegister = destination,
    ...
    for (const auto &operation : block.operations) {
        ...
        case Opcode::LockedOrGuestMemory:
            ...
            if (operation.width != Width::I8 && operation.width != Width::I16 &&
                ...
                operation.width != Width::I64) {
                errors.emplace_back(
                    "locked_or_guest_memory currently requires i8, i16, i32, or i64");
            }
            break;
        case Opcode::LockedAndGuestMemory:
            ...
            if (operation.width != Width::I16 && operation.width != Width::I32 &&
                operation.width != Width::I64) {
                errors.emplace_back(
                    "locked_and_guest_memory currently requires i16, i32, or i64");
            }
            break;
        ...
        case Opcode::UnpackHighPackedSingleXmm:
            if (!operation.guestXmmRegister ||
                ...
                operation.width != Width::I64) {
                errors.emplace_back(
                    "unpack_high_packed_single_xmm has incomplete registers");
            }
            break;
        case Opcode::UnpackHighGuestPackedSingleXmm:
            checkUse(operation.lhs, "guest address");
            if (!operation.guestXmmRegister || operation.width != Width::I64) {
                errors.emplace_back(
        ...
        case Opcode::ConvertIntToDoubleXmm:
            checkUse(operation.lhs, "integer value");
            if (!operation.guestXmmRegister ||
                (operation.width != Width::I32 &&
                 operation.width != Width::I64)) {
                errors.emplace_back(
        ...
        case Opcode::UpdateAdcFlags:
            ...
            if (operation.width != Width::I8 && operation.width != Width::I32 &&
                operation.width != Width::I64) {
                errors.emplace_back("update_adc_flags requires i8, i32 or i64");
            }
            break;
