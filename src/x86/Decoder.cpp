...
std::vector<DecodedInstruction> Decoder::decodeBlock(std::span<const std::uint8_t> code,
    ...
    while (cursor < code.size()) {
        ...
        if (code[cursor] == 0x66U && code.size() - cursor >= 3 &&
            code[cursor + 1] == 0x0FU && code[cursor + 2] == 0x76U) {
            if (code.size() - cursor < 4) {
                throw DecodeError(address, remaining,
        ...
        if (code[cursor] == 0xF3U && code.size() - cursor >= 3 &&
            ...
            if (mode > 0x2U || rmEncoding == 0x4U ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only MOVQ xmm, qword [base+disp8/disp32] is supported");
            }
            auto operandCursor = cursor + 4;
            std::int64_t displacement = 0;
            if (mode == 0x1U) {
        ...
        if (code[cursor] == 0x66U && code.size() - cursor >= 3 &&
            ...
            if (mode > 0x2U || (mode == 0 && rmEncoding == 0x5U)) {
                throw DecodeError(
                    address, remaining,
                    "only MOVDQA xmm, [base+index*scale+disp] memory operands are supported");
            }
            cursor += 4;
            auto baseEncoding = rmEncoding;
            std::optional<Register> index;
            ...
            instruction.operands.push_back(XmmRegisterOperand{static_cast<XmmRegister>(
                static_cast<std::uint8_t>((modrm >> 3U) & 0x7U))});
            instruction.operands.push_back(MemoryOperand{
                decodeRegister(baseEncoding, false), displacement, 128, index,
                scale});
        ...
        if (code[cursor] == 0xF3U && code.size() - cursor >= 5 &&
            ...
            if (rexW || rexX || mode == 0x3U || rmEncoding == 0x4U ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only REX-extended MOVDQU [base+disp8/disp32], xmm is supported");
            }
            auto operandCursor = cursor + 5;
            std::int64_t displacement = 0;
            if (mode == 0x1U) {
        ...
        if (code[cursor] == 0xF0U && code.size() - cursor >= 2 &&
            ...
            if (cursor >= code.size()) {
                throw DecodeError(address, remaining,
                                  "truncated LOCK INC memory operand");
            }
            const auto modrm = code[cursor++];
            const auto mode = static_cast<std::uint8_t>((modrm >> 6U) & 0x3U);
            const auto extension =
            ...
            if (mode == 0x3U || extension != 0 || rmEncoding == 0x4U ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only LOCK INC dword [base+disp8/disp32] is supported");
            }
            std::int64_t displacement = 0;
            if (mode == 0x1U) {
                if (cursor >= code.size()) {
                    throw DecodeError(address, remaining,
                                      "truncated LOCK INC disp8");
                }
                displacement = std::bit_cast<std::int8_t>(code[cursor++]);
            } else if (mode == 0x2U) {
                if (code.size() - cursor < 4) {
                    throw DecodeError(address, remaining,
                                      "truncated LOCK INC disp32");
                }
                displacement = readI32(code.subspan(cursor, 4));
                cursor += 4;
            }
            instruction.opcode = Opcode::LockIncMem;
            instruction.operands.push_back(MemoryOperand{
                decodeRegister(rmEncoding, false), displacement, 32});
            const auto length = cursor - instructionStart;
            instruction.length = static_cast<std::uint8_t>(length);
        ...
        if (code[cursor] == 0xF0U && code.size() - cursor >= 3 &&
            code[cursor + 1] == 0x0FU && code[cursor + 2] == 0xC1U) {
            cursor += 3;
            if (cursor >= code.size()) {
                throw DecodeError(address, remaining,
                                  "truncated LOCK XADD dword memory operand");
            }
            const auto modrm = code[cursor++];
            const auto mode = static_cast<std::uint8_t>((modrm >> 6U) & 0x3U);
            const auto regEncoding =
            ...
            if (mode == 0x3U || rmEncoding == 0x4U ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only LOCK XADD dword [base+disp8/disp32], r32 is supported");
            }
            std::int64_t displacement = 0;
            if (mode == 0x1U) {
                if (cursor >= code.size()) {
        ...
        if (code[cursor] == 0xF0U) {
            if (code.size() - cursor < 4 || code[cursor + 1] != 0x0FU ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only LOCK CMPXCHG or LOCK XADD r/m32, r32 is supported from prefix F0");
            }
            cursor += 3;
            const auto modrm = code[cursor++];
            const auto mode = static_cast<std::uint8_t>((modrm >> 6U) & 0x3U);
        ...
        if (opcode == 0x24U) {
            ...
            if (mode > 0x2U || (mode == 0 && rmEncoding == 0x5U)) {
                throw DecodeError(
                    address, remaining,
                    "only OR r32, dword [base+index*scale+disp8/disp32] is supported");
            }
            auto baseEncoding = rmEncoding;
            std::optional<Register> index;
            std::uint8_t scale = 1;
            ...
            instruction.operands.push_back(MemoryOperand{
                decodeRegister(baseEncoding, rexB), displacement, 32, index,
                scale});
            ...
            if (!rexW ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only MOVSXD r64, r32/dword [base+index*scale+disp] is supported");
            }
            if (mode == 0x3U) {
            ...
            if (mode == 0x3U) {
                ...
                instruction.operands.push_back(MemoryOperand{
                    decodeRegister(baseEncoding, rexB), displacement, 8,
                    index, scale});
                instruction.operands.push_back(source);
            }
            ...
            if (rexX || mode > 0x2U || rmEncoding == 0x4U ||
                ...
                throw DecodeError(
                    address, remaining,
                    "only XCHG dword/qword [base+disp8/disp32], register is supported");
            }
            std::int64_t displacement = 0;
            if (mode == 0x1U) {
                if (cursor >= code.size()) {
            ...
            const auto extension = static_cast<std::uint8_t>((modrm >> 3U) & 0x7U);
            const auto rmEncoding = static_cast<std::uint8_t>(modrm & 0x7U);
            if (extension == 0x0U && mode <= 0x2U && !rexR && !rexX &&
                rmEncoding != 0x4U &&
                !(mode == 0 && rmEncoding == 0x5U)) {
                std::int64_t displacement = 0;
                if (mode == 0x1U) {
            ...
            const auto extension = static_cast<std::uint8_t>((modrm >> 3U) & 0x7U);
            const auto rmEncoding = static_cast<std::uint8_t>(modrm & 0x7U);
            if (extension == 0x7U && mode <= 0x2U && !rexR) {
                const bool ripRelative =
                    mode == 0 && rmEncoding == 0x5U && !rexB;
                auto base = decodeRegister(rmEncoding, rexB);
                std::optional<Register> index;
            ...
            if (!rexW && extension != 0x0U && extension != 0x1U &&
                ...
                extension != 0x7U) {
                throw DecodeError(address, remaining,
                                  "only 32-bit ADD /0, OR /1, AND /4, SUB /5, XOR /6, and CMP /7 are supported from legacy opcode 83");
            }
            if (mode != 0x3U || rexR || rexX ||
                (extension != 0x0U && extension != 0x1U && extension != 0x4U && extension != 0x5U && extension != 0x6U &&
                 extension != 0x7U)) {
                throw DecodeError(
                    address, remaining,
                    "only register-direct ADD /0, OR /1, AND /4, SUB /5, XOR /6, and CMP /7 from opcode 83 are supported");
            }
            const auto immediate = std::bit_cast<std::int8_t>(code[cursor++]);
