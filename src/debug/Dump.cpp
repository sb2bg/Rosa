...
std::string dumpX86(std::span<const x86::DecodedInstruction> instructions) {
    ...
    for (const auto &instruction : instructions) {
        ...
        case x86::Opcode::AndMemReg: {
            const auto memory =
                std::get<x86::MemoryOperand>(instruction.operands[0]);
            stream << (memory.width == 16    ? "and word ["
        ...
        case x86::Opcode::BitTestMemImm: {
            const auto memory =
                std::get<x86::MemoryOperand>(instruction.operands[0]);
            stream << "bt " << (memory.width == 32 ? "dword [" : "qword [")
                   << x86::registerName(memory.base);
            if (memory.index) {
                stream << '+' << x86::registerName(*memory.index);
                if (memory.scale != 1) {
        ...
        case x86::Opcode::PinsrwXmmReg: {
            ...
            if (!fromMemory) {
                ...
                const auto memory =
                    std::get<x86::MemoryOperand>(instruction.operands[1]);
                stream << "word [" << x86::registerName(memory.base);
                if (memory.displacement < 0) {
                    stream << "-0x" << -memory.displacement;
                } else if (memory.displacement > 0) {
                    stream << "+0x" << memory.displacement;
        ...
        case x86::Opcode::MovdqaRegReg:
            stream << (instruction.opcode == x86::Opcode::MovapdRegReg
                           ? "movapd "
                       : instruction.opcode == x86::Opcode::MovapsRegReg
                           ? "movaps "
                           : "movdqa ")
                   << x86::xmmRegisterName(
                          std::get<x86::XmmRegisterOperand>(
...
std::string dumpIr(const ir::Block &block) {
    ...
    for (const auto &operation : block.operations) {
        ...
        case ir::Opcode::UnpackLowPackedSingleXmm:
            stream << "unpack_low_packed_single_xmm.i64 "
                   << x86::xmmRegisterName(*operation.guestXmmRegister) << ", "
                   << x86::xmmRegisterName(
                          *operation.sourceGuestXmmRegister);
            break;
        case ir::Opcode::UnpackLowGuestPackedSingleXmm:
            stream << "unpack_low_guest_packed_single_xmm " << valueName(*operation.lhs)
                   << ", " << x86::xmmRegisterName(*operation.guestXmmRegister);
            break;
        ...
        case ir::Opcode::ComparePackedDoubleXmm:
            stream << "compare_packed_double_xmm.i64 "
                   << x86::xmmRegisterName(*operation.guestXmmRegister)
                   << ", "
                   << x86::xmmRegisterName(
                          *operation.sourceGuestXmmRegister)
        ...
        case ir::Opcode::ConvertInt32x2ToDoubleXmm:
            stream << "convert_int32x2_to_double_xmm.i64 " << valueName(*operation.lhs)
                   << ", " << x86::xmmRegisterName(*operation.guestXmmRegister);
            break;
