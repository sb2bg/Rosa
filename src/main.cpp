...
void printUsage(std::ostream &stream) {
    stream << "usage:\n"
              ...
              "  rosa run --dyld <x86_64-dyld> <x86_64-mach-o> [dump options]\n";
}
...
int runDyldExperiment(const std::filesystem::path &executablePath,
                      const std::filesystem::path &dyldPath, const DumpOptions &options) {
    ...
    std::cout << "dyld experiment: app-entry=0x" << std::hex << executable.entryPoint.value
              << " dyld-entry=0x" << dyld.entryPoint.value << " initial-rsp=0x" << state.rsp
              << std::dec << '\n';
...
int main(int argc, char **argv) {
    try {
        ...
        if (command == "run") {
            ...
            if (options.dyld) {
                return runDyldExperiment(options.executable, *options.dyld, options.dumps);
            }
